...
#Iremos criar um data frame sem a variavel resposta vendas do cafe, pois ela sera 
estimada pela equacao de regressao que ajustamos
...
Análise Estatística de Dados - Página 148 de 177
fit2 <- lm(Vendas_Cafe ~ Preco_Cafe + Promocao + Preco_Leite + nova_variavel, 
           data = dados)
summary(fit2) #Observe o p valor da nova variavel, nao e significativo
fit2 <- step(fit2, direction = 'both')
summary(fit2) #Observe que o stepwise removeu a nova variave